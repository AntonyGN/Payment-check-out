<div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                ...
                @foreach($items as $item)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            ...
                            <!-- Check if the item is active before showing the Check Out button -->
                              @if($item->status)
                                <x-jet-button wire:click="confirmCheckOut( {{ $item->id}})" class="bg-green-500 hover:bg-green-700">
                                    Check Out
                                </x-jet-button>
                            @endif
                        </td>
    ...
    <x-jet-confirmation-modal wire:model="confirmingCheckOut">
        <x-slot name="title">
            {{ __('Check Out Item') }}
        </x-slot>
    
        <x-slot name="content">
            {{ __('Are you sure you want to check out this item?') }}
        </x-slot>
    
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmingCheckOut', false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>
    
            <x-jet-button class="ml-2" wire:click="checkOutItem" wire:loading.attr="disabled" class="bg-green-500 hover:bg-green-700">
                {{ __('Check Out') }}
            </x-jet-button>
        </x-slot>
    </x-jet-confirmation-modal>
